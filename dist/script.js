...
window.addEventListener('DOMContentLoaded', function () {
  var pageSlider = new _modules_sliders_mainSlider__WEBPACK_IMPORTED_MODULE_0__["default"]({
    container: '.page',
    buttons: '.next'
  });
  pageSlider.render();
    ...
    value: function init() {
      ...
      this.btns.forEach(function (btn) {
        btn.addEventListener('click', function () {
          ...
          if (_this.btn.nextElementSibling.style.display === 'none') {
            _this.btn.nextElementSibling.style.display = 'block';
          } else {
            _this.btn.nextElementSibling.style.display = 'none';
          }
        });
      });
    }
...
function () {
  ...
  _createClass(Difference, [{
    key: "bindTriggers",
    value: function bindTriggers(row, rowItems, counter) {
    ...
    key: "hideItems",
    value: function hideItems(rowItems) {
      rowItems.forEach(function (item, i, arr) {
        if (i !== arr.length - 1) {
...
function (_Slider) {
  ...
  _createClass(MainSlider, [{
    ...
    value: function showSlide(n) {
      ...
      try {
        ...
        if (this.slideIndex === 3) {
          ...
          setTimeout(function () {
            _this.timeBlock.style.opacity = '1';

            _this.timeBlock.classList.add('slideInUp');
          }, 3000);
        } else {
    ...
    value: function bindTriggers() {
      ...
      this.prevAll.forEach(function (item) {
      ...
      this.nextAll.forEach(function (item) {
        item.addEventListener('click', function (e) {
          e.stopPropagation();
          e.preventDefault();
...
function (_Slider) {
  ...
  function MiniSlider(container, prev, next, activeClass, animated, auto) {
    _classCallCheck(this, MiniSlider);

    return _possibleConstructorReturn(this, _getPrototypeOf(MiniSlider).call(this, container, prev, next, activeClass, animated, auto));
  }
  ...
  _createClass(MiniSlider, [{
    ...
    value: function bindTriggers() {
      ...
      var sliderBtns = this.container.querySelectorAll('button');
      this.prev.addEventListener('click', function () {
        if (sliderBtns) {
          sliderBtns.forEach(function (btn) {
            _this.container.insertBefore(btn, _this.slides[0]);
          });
        }

        _this.container.insertBefore(_this.slides[_this.slides.length - 1], _this.slides[0]);
...
var Slider = function Slider() {
  var _ref = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {},
      ...
      _ref$prevAll = _ref.prevAll,
      prevAll = _ref$prevAll === void 0 ? null : _ref$prevAll,
      _ref$nextAll = _ref.nextAll,
      nextAll = _ref$nextAll === void 0 ? null : _ref$nextAll,
...
/*#__PURE__*/
function () {
  function VideoPlayer(buttons, overlay) {
  ...
  _createClass(VideoPlayer, [{
    ...
    value: function bindTriggers() {
      ...
      this.btns.forEach(function (btn, i) {
        try {
          if (i % 2) {
            _this.btns[i].closest('.module__video-item').setAttribute('data-disabled', 'true'); //дизейблим каждый второй блок

          }

          btn.addEventListener('click', function () {
            if (!btn.closest('.module__video-item') || !btn.closest('.module__video-item').getAttribute('data-disabled')) {
              //первое условие для не модульной страницы
              _this.activeBtn = btn; //получаем, что бы обрабатывать в онСтэйдж функции

              if (document.querySelector('iframe#frame')) {
                //проверка, был ли создан плеер
    ...
    value: function onPlayerStateChange(state) {
      //запускается сама из обьекта плеера
      try {
        var blockedBlock = this.activeBtn.closest('.module__video-item').nextElementSibling;
        var unblockedIcon = this.activeBtn.querySelector('svg').cloneNode(true);
